...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) : SingleLinkedList() {
        ...
        tmp.init_func(values.begin(), values.end());
        swap(tmp);
    }
    SingleLinkedList(const SingleLinkedList& other) :SingleLinkedList() {
        SingleLinkedList<Type> tmp;
        tmp.init_func(other.begin(), other.end());
        swap(tmp);
    }
...
private:
    ...
    void init_func(const Iterator& begin, const Iterator& end) {
        Node* k = &head_;
        for (auto it = begin; it != end; ++it) {
            k->next_node = new Node(*it, nullptr);
            k = k->next_node;
            ++size_;
        }
    }
